...
public class FishbookUIController : UIController
{
	...
	{
		this.windowController = (SingletonMonoBehaviour<UIFactory>.Instance.Create((UI.PartTypeEnum) 501, base.transform) as FishbookWindowUIPartController);
		...
		var fishList = Fishbook.Instance?.GetFishesForPoint(pointInfo.PointId);
		if (fishList == null) {
		...
		this.mButtonInfo = (SingletonMonoBehaviour<UIFactory>.Instance.Create(UI.PartTypeEnum.ButtonInfo, this.windowController.InputArea.transform) as ButtonInfoUIPartController);
